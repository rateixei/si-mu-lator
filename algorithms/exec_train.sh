...
#singularity exec --nv -B /sdf,/gpfs,/scratch ${SING_IMG} python train.py -f "${DATA_LOC}" -m "tcn" --task "regression" --lambda 1  # --detector ${DET}
singularity exec --nv -B /sdf,/gpfs,/scratch ${SING_IMG} python train.py -f "${DATA_LOC}" -m "tcn" --task "regression" --lambda 1  --do-angle # --detector ${DET}
