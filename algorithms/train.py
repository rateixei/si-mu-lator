...
if args.mod_type == 'deepsets':
    ...
    my_model = mlmodels.model_tcn_muon(input_shape=(X.shape[1],X.shape[2]),
        ...
        F_layers=[20,10], 
        batchnorm=args.bnorm, 
        do_reg_out=args.task_type=='regression',
        ll=0.)
else:
    print("!!!")
    sys.exit()
...
if args.task_type=='regression':
    ...
    X_train, Y_clas_train, Y_reg_train = shuffle(X, Y_mu, data_ev_mu_x)
    Y_train = np.zeros( (Y_clas_train.shape[0], 2 ) ) 
    Y_train[:,0] = Y_clas_train
    Y_train[:,1] = Y_reg_train
